...
import { TbBusStop,TbSunset2 } from "react-icons/tb";
import GetApiCall from "../../GetApi/GetApiCall";
import Footer from "../../Footer/Footer";
import { WiSunrise } from "react-icons/wi";
import { IoCloudyNightSharp,IoTicket, IoCloudyNightOutline } from "react-icons/io5";
...
const GetBus = () => {
  ...
  const [seatMessage, setSeatMessage] = useState<boolean>(false)
   const url = `${process.env.REACT_APP_apiURL}/userBus/viewBuses?startingPoint=${encodeURIComponent(h.startingPoint)}&destination=${encodeURIComponent(h.destination)}`
  ...
  const ViewSeats = (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
    ...
    if (date) {
      const checkSeat = GetApiCall(`${process.env.REACT_APP_apiURL}/userBus/viewSeats?date=${date}`)
      checkSeat.then(res => {
  ...
  function booking() {
    if (seatDetails.length > 0) {
      navigate('/bookingProcess', { state: { busId: busId, date: date, noOfSeats: noOfSeats, totalAmount: totalAmount, status: 'status', seats: seatDetails } })
    } else {
          ...
          <div className="buses">
            {
              ...
              (
                <div>
                <p style={{fontSize: "45px"}}>No Buses available</p>
                </div>
              ) 
            }
